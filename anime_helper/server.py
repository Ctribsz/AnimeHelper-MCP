...
def season_top(kind: str = "ANIME", season: str | None = None, year: int | None = None,
               sort: str = "TRENDING_DESC", limit: int = 10):
    """
    ...
    try:
        ...
        if kind == "ANIME":
            q = """
            query ($type: MediaType!, $season: MediaSeason!, $year: Int!, $per: Int!, $sort: [MediaSort!]!) {
              Page(perPage: $per) {
            ...
            data = gql(q, {"type": "ANIME", "season": sea, "year": yr, "per": limit, "sort": [sort]})
            hits = [norm_hit_from_anilist(m) for m in data["Page"]["media"]]
            return {"schemaVersion": SCHEMA, "kind": "ANIME", "season": sea, "year": yr, "sort": sort, "results": hits[:limit]}
...
def ask(text: str, default_kind: str = "ANIME", default_limit: int = 5):
    ...
    - season_top (esta temporada)
    - trending (tendencias / populares)
    - search_media (+ media_details si piden ficha/detalles)
    ...
    try:
        ...
        # 2) Temporada actual / esta temporada (ANIME)
        if ("temporada" in low and ("actual" in low or "esta" in low)) or \
           re.search(r"\b(this|current)\s+season\b", low):
            res = season_top(kind="ANIME", limit=limit)
            return {"schemaVersion": SCHEMA, "intent": "season_top", "args": {"kind": "ANIME", "limit": limit}, "result": res}

        # 3) Tendencias / populares / llamativos
        if any(w in low for w in ["tendencia", "tendencias", "trending", "populares", "más llamativos", "mas llamativos"]):
            res = trending(kind=kind, limit=limit)
            return {"schemaVersion": SCHEMA, "intent": "trending", "args": {"kind": kind, "limit": limit}, "result": res}

        # 4) Detalles / ficha de X
        if any(w in low for w in ["detalles", "detalle", "ficha", "info de", "información de", "informacion de", "about"]):
            ...
            # mejor id (preferimos AniList si está)
            best = None
...
def help():
    ...
    return {
        ...
        "features": [
            "ask: lenguaje natural → enruta a season_top / trending / search / details / airing_status",
            "search_media: búsqueda por título (ANIME/MANGA)",
            "media_details: ficha normalizada + recomendaciones",
            "trending: top en tendencia",
            "season_top: top de la temporada actual (ANIME)",
            "airing_status: último/próximo episodio (ANIME)"
        ],
        "examples": [
            { "title": "Pregunta en lenguaje natural", "prompt": "¿En qué capítulo va One Piece?" },
            { "title": "Temporada actual", "prompt": "¿Cuáles son los animes más llamativos de esta temporada?" },
            { "title": "Detalles de un título", "prompt": "Dame la ficha de Vinland Saga" },
